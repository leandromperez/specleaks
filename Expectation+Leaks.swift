...
extension Expectation where T==LeakTestConstructor{
    ...
    public func toNotLeakWhen<P>( shouldFail: Bool = false, _ action : (P)->()) where P:AnyObject {
        do{
            guard let constructor = try expression.evaluate() else{
                fail()
                return
    ...
    public func toLeakWhen<P>(shouldFail: Bool = false, _ action : (P)->()) where P:AnyObject {
